...
// Client Side Image Compression Helper with Fail-Safe Fallbacks
export function compressImage(file: File, maxW = 800, maxH = 800): Promise<string> {
  return new Promise((resolve) => {
    ...
    reader.onload = (event) => {
      ...
      img.onerror = () => {
        // Fallback to raw data URL if Image element fails to load
        resolve(rawDataUrl);
      };
...
// Authentication & Password Management for Dra. Aline
const AUTH_CREDENTIALS_KEY = 'fisio_aline_credentials';
const AUTH_SESSION_KEY = 'fisio_aline_session';

export interface AuthCredentials {
  email: string;
  passwordHash: string; // Plain/stored string for clinic access
  updatedAt: string;
}
...
export function getAuthCredentials(): AuthCredentials {
  ...
  // Default initial credentials for Dra. Aline
  const defaultCreds: AuthCredentials = {
    email: 'aline@fisio.com.br',
    passwordHash: '1234',
    updatedAt: new Date().toISOString()
  };
  localStorage.setItem(AUTH_CREDENTIALS_KEY, JSON.stringify(defaultCreds));
  return defaultCreds;
}

export function setAuthCredentials(email: string, passwordHash: string) {
...
export function verifyLogin(inputIdentifier: string, inputPass: string): boolean {
  const creds = getAuthCredentials();
  const inputClean = inputIdentifier.trim().toLowerCase();
  const passClean = inputPass.trim();

  // Allow login by email or any variation of username for Dra. Aline Franchi Modesto
  const validUsernames = [
    creds.email.toLowerCase(),
    'aline',
    'dra.aline',
    'dra.alinefranchimodesto',
    'aline.modesto',
    'dr.aline',
    'dra.aline'
  ];

  const isUsernameMatch = validUsernames.some(u => inputClean === u || inputClean.includes('aline'));
  const isPasswordMatch = passClean === creds.passwordHash;

  return isUsernameMatch && isPasswordMatch;
}

export function setAuthSession(isLoggedIn: boolean) {
  if (isLoggedIn) {
