...
export default function PatientsTab({
  ...
  const handleUpdatePatientSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedPatient || !editPatientName.trim()) return;
  ...
  return (
    <div className="space-y-4 font-sans text-slate-800">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        {/* COLUNA ESQUERDA: Busca e Lista de Pacientes */}
        <div className={`lg:col-span-1 space-y-4 ${selectedPatientId ? 'hidden lg:block' : 'block'}`}>
          <div className="flex justify-between items-center gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full bg-white pl-10 pr-4 py-2.5 text-xs border border-slate-100 rounded-xl focus:outline-none focus:border-[#dfaba0] focus:ring-1 focus:ring-[#dfaba0] shadow-sm"
              />
            </div>
            <button
              onClick={() => setShowAddModal(true)}
              className="p-2.5 bg-[#dfaba0] hover:bg-[#d4998e] active:scale-95 text-white rounded-xl shadow-md shadow-red-100 transition-transform shrink-0"
          ...
          <div className="space-y-2.5 max-h-[calc(100vh-180px)] overflow-y-auto no-scrollbar pb-10">
            {filteredPatients.length === 0 ? (
              <div className="text-center p-8 bg-white rounded-3xl border border-dashed border-slate-200 shadow-sm">
                <User className="w-8 h-8 text-slate-300 mx-auto mb-2" />
                <p className="text-xs font-semibold text-slate-500">Nenhum paciente cadastrado.</p>
              </div>
            ) : (
              filteredPatients.map((p) => {
                ...
                return (
                  <div
                    ...
                    className={`p-4 rounded-2xl border flex justify-between items-center transition-all cursor-pointer active:scale-[0.99] duration-100 shadow-sm ${
                      isSelected
                        ? 'bg-[#faf3f0] border-[#dfaba0] ring-1 ring-[#dfaba0]/30'
                        : 'bg-white border-slate-50 hover:border-[#dfaba0]/20'
                    }`}
                  >
                    <div className="flex gap-3 items-center min-w-0">
                      <div className="w-10 h-10 rounded-xl bg-white border border-slate-100 flex items-center justify-center text-[#dfaba0] shrink-0 font-bold font-serif text-sm">
                        {p.name.charAt(0)}
                      </div>
                    ...
                    <ChevronRight className={`w-4 h-4 transition-transform ${isSelected ? 'text-[#dfaba0] translate-x-1' : 'text-slate-300'}`} />
                  </div>
                );
              })
            )}
        ...
        {/* COLUNA DIREITA: Detalhes do Prontuário do Paciente */}
        <div className={`lg:col-span-2 ${selectedPatientId ? 'block' : 'hidden lg:block'}`}>
          {selectedPatient ? (
            <div className="space-y-4 lg:max-h-[calc(100vh-100px)] overflow-y-auto no-scrollbar pb-16">
              {/* Patient Mini Banner */}
              <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm space-y-3 relative">
                <button
                  onClick={() => setSelectedPatientId(null)}
                  className="absolute top-4 right-4 text-xs font-bold text-[#dfaba0] hover:underline lg:hidden"
                >
                  ← Voltar
                </button>
                ...
                <button
                  onClick={() => setSelectedPatientId(null)}
                  className="absolute top-5 right-5 text-[10px] font-extrabold text-slate-400 hover:text-red-500 uppercase tracking-wider hidden lg:block"
                >
                  Fechar Prontuário [x]
                </button>

                <div className="flex gap-3.5 items-center">
                  <div className="w-14 h-14 rounded-2xl bg-[#faf3f0] border border-slate-100 flex items-center justify-center text-[#dfaba0] font-bold font-serif text-xl shrink-0">
                    {selectedPatient.name.charAt(0)}
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h3 className="font-serif text-lg font-bold text-[#2c2523] leading-tight truncate">{selectedPatient.name}</h3>
                      <button
                        type="button"
                        onClick={() => {
                          setEditPatientName(selectedPatient.name);
                          setEditPatientPhone(selectedPatient.phone || '');
                          setEditPatientEmail(selectedPatient.email || '');
                          setEditPatientBirthDate(selectedPatient.birthDate || '');
                          setEditPatientCpf(selectedPatient.cpf || '');
                          setShowEditPatientModal(true);
                        }}
                        className="p-1 text-slate-400 hover:text-[#dfaba0] hover:bg-slate-50 rounded-lg transition-colors inline-flex items-center justify-center shrink-0"
                    ...
                    <p className="text-[11px] text-slate-500 mt-1 flex items-center gap-1 font-semibold">
                      <Calendar className="w-3.5 h-3.5 text-[#dfaba0]" />
                ...
                <div className="grid grid-cols-2 gap-2 pt-3 border-t border-slate-100 text-xs text-slate-500 font-medium">
                  <p className="flex items-center gap-1.5 truncate">
                    <Phone className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                    <strong>WhatsApp:</strong> {selectedPatient.phone || 'Não informado'}
                  </p>
                  <p className="flex items-center gap-1.5 truncate">
                    <CreditCard className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                    <strong>CPF:</strong> {selectedPatient.cpf || 'Não informado'}
                  </p>
                  <p className="col-span-2 flex items-center gap-1.5 truncate">
                    <Mail className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                    <strong>E-mail:</strong> {selectedPatient.email || 'Não informado'}
                  </p>
                </div>

                {/* LGPD Protection Status Card */}
                <div className={`p-3.5 rounded-2xl border flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 text-xs ${
                  ...
                  >
                    {selectedPatient.lgpdAccepted ? 'Ver Termo' : 'Gerar Termo'}
                  </button>
                </div>
                ...
                <button
                  ...
                  className="w-full py-2 bg-red-50 hover:bg-red-100 text-red-600 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1 shadow-sm border border-red-200"
                >
                  <Trash2 className="w-3.5 h-3.5" /> Excluir Prontuário do Paciente
                </button>
              </div>

              {/* Segmented Controller for Patient Details */}
              <div className="flex gap-1 bg-white p-1 rounded-2xl border border-slate-100 shadow-sm text-xs font-bold text-slate-500">
                <button
                  type="button"
                  ...
                  className={`flex-1 py-2.5 rounded-xl flex items-center justify-center gap-1.5 transition-all ${
                    patientDetailTab === 'anamnese'
                      ? 'bg-[#faf3f0] text-[#2c2523] shadow-sm font-extrabold'
                      : 'hover:text-slate-700'
                  }`}
                >
                  <FileText className="w-3.5 h-3.5 text-[#dfaba0]" />
                  Anamnese
                </button>
                <button
                  type="button"
                ...
                >
                  <Camera className="w-3.5 h-3.5 text-[#dfaba0]" />
                  Evoluções & Fotos
                </button>
                <button
                  type="button"
            ...
            <div className="hidden lg:flex flex-col items-center justify-center p-12 bg-white rounded-3xl border border-dashed border-slate-200 h-[calc(100vh-160px)] text-center shadow-sm">
              <User className="w-16 h-16 text-slate-300 mb-4 animate-pulse" />
              <h3 className="font-serif text-lg font-bold text-[#2c2523]">Nenhum Paciente Selecionado</h3>
              <p className="text-xs text-slate-500 max-w-sm mt-1.5 leading-relaxed">
                Selecione um paciente na lista lateral esquerda para abrir a ficha de anamnese completa, evoluções fotográficas de antes e depois e o histórico de sessões.
              </p>
            </div>
          )}
        </div>
      ...
      {showEditPatientModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <form 
            ...
            className="bg-white rounded-3xl p-6 max-w-xl w-full shadow-2xl space-y-4 text-slate-800"
          >
            <div className="flex items-center gap-2 border-b border-slate-50 pb-2">
              <User className="w-5 h-5 text-[#dfaba0]" />
              <h3 className="font-serif text-lg font-bold text-slate-800">Editar Cadastro do Paciente</h3>
            </div>
